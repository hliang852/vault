...
## 7. v2 changes (2026/07/06)

- **Two-track architecture adopted.** Track 1: this masterfile = curated landmark knowledge bank for case-relations study; additions by **manual trigger only** (a deal mention → researched, cited row). Track 2: `Japan_new.csv` = append-only daily scanner output (EDINET + TDnet + news layer) with no landmark judgment; rumor-stage records labeled `Media report / rumored`. No auto-append from Track 2 to Track 1.
- **Citation columns added** to `Japan_Master.csv`: `Source URL 1 (primary)`, `Source URL 2`, `Source URL 3` — one citation per column. Historical rows carry `TBV — citation backfill pending` until the batch verification passes complete. Source hierarchy and rules: `Japan_Sources_Methodology.md`.
- **Selection criteria recorded** in `Japan_Landmark_Criteria.md` (criteria C1–C6, exclusions, promotion rules).
- **Scope expansion approved** to 2015/01/01–2026/06/30; candidate universe for sign-off in `Japan_Expansion_Candidates_2015-2022.csv` (pending verification; nothing enters the masterfile uncited).
- `Japan.csv` (ML-ready file) will be regenerated with the new citation columns once backfill batch 1 lands; regenerating against empty citation columns was deemed pointless.
