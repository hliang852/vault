...
**Status: design phase started (2026-07-07). Design mockup + content plug-in spec produced; the real site is not yet built.**

`viewer/Japan_Precedent_Constellation.html` (the pre-existing viewer) still renders the Part 2 graph with Explore and Find-Precedent modes; it got only the `short_name` label fix on 2026-07-06 and is otherwise untouched -- it will eventually be superseded by the new site below.
...
- Maintainer review of the mockup -> iterate on the design or approve it, then build the real interactive site (live force layout, animated zoom transitions between the three layers, working finder wired to the full Part 2 feature set, playbook panels generated per cluster from the axis tags).
- Decide the finder's final input set (the mockup shows archetype/industry/competing-bid/board-posture + fact pills; whether axis values like instigator type or lock-up count should be directly selectable is open).
- A build step that generates the site's embedded data from `output/precedent_graph_data.json` + `content/cases/*.md` automatically (replaces today's manual copy-paste; rendering/validation rules already specified in `Case_Content_Spec.md`).
